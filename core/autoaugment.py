...
class FullyRandomAutoAugment(object):
    def __init__(self):
        self.policies = [
            'ShearX',
            'ShearY',
    ...
    def __call__(self, img):
        policy_ids = np.random.choice(len(self.policies),2)
        policy_prob = np.random.rand(2)
        policy_magnitude = np.random.randint(0,NUM_MAGNITUDES-1,2)
        arg = [self.policies[policy_ids[0]], policy_prob[0], policy_magnitude[0],
               self.policies[policy_ids[1]], policy_prob[1], policy_magnitude[1]]
        img = apply_policy(img, arg)
        return img


class RandomAutoAugment(object):
    def __init__(self):
        self.policies = [
            ['Invert',      'Contrast'],
            ['Rotate',      'TranslateX'],
            ...
            ['TranslateY',  'Sharpness'],
            ['Brightness',  'Color'],
            ['Solarize',    'Invert'],
            ['Equalize',    'AutoContrast'],
            ['Equalize',    'Equalize'],
            ['Color',       'Equalize'],
            ['AutoContrast','Solarize'],
            ['Brightness',  'Color'],
            ['Solarize',    'AutoContrast'],
            ['TranslateY',  'TranslateY'],
            ['AutoContrast','Solarize'],
            ['Equalize',    'Invert'],
            ['TranslateY',  'AutoContrast'],
        ]

    def __call__(self, img):
        policy = self.policies[np.random.randint(0,len(self.policies))]
        policy_prob = np.random.rand(2)
        policy_magnitude = np.random.randint(0,NUM_MAGNITUDES-1,2)
        arg = [policy[0], policy_prob[0], policy_magnitude[0],
               policy[1], policy_prob[1], policy_magnitude[1]]
...
def shear_x(img, magnitude):
    img = np.array(img)
    magnitudes = np.linspace(-0.3, 0.3, NUM_MAGNITUDES)
...
def translate_x(img, magnitude):
    img = np.array(img)
    magnitudes = np.linspace(-150/331, 150/331, NUM_MAGNITUDES)
...
def rotate(img, magnitude):
    img = np.array(img)
    magnitudes = np.linspace(-30, 30, NUM_MAGNITUDES)
...
def cutout(org_img, magnitude=None):
    ...
    magnitudes = np.linspace(0, 60/331, NUM_MAGNITUDES)
